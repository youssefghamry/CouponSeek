...
namespace Elementor;
/**
 * Elementor Product Category List Widget.
 *
 * Elementor widget that inserts a list displaying WooCommerce Categories.
 *
 * @since 1.0.0
 */
class SSD_Elementor_Product_Category_List_Widget extends \Elementor\Widget_Base {

	public function get_name() {
		return 'ssd_product_category_list';
	}

	public function get_title() {
		return __( 'Product Categories List', 'couponseek' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'list_title',
			[
				'label' => __( 'List Title', 'couponseek' ),
		...
		$this->start_controls_section(
			'style_section',
			[
				'label' => __( 'Categories List', 'couponseek' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em' ],
				'selectors' => [
					'{{WRAPPER}} .ProductCategories .product-categories-list' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_control(
			'list_categories_gap',
			[
				'label' => __( 'Categories Gap', 'couponseek' ),
				...
				'selectors' => [
					'{{WRAPPER}} .ProductCategories .product-category' => 'margin-bottom: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .ProductCategories .product-child-categories' => 'margin-top:{{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .ProductCategories .product-categories-list' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ProductCategories .product-category .category-icon' => 'width: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .ProductCategories .product-category .category-icon + .product-category-meta' => 'margin-left: {{SIZE}}{{UNIT}};min-height: {{SIZE}}{{UNIT}};line-height: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	protected function render() {
		...
		<div class="ProductCategories">
			...
			<div class="product-categories-title <?php echo esc_attr($title_alignment); ?>">
				<h4><?php echo wp_kses_post($settings['list_title']); ?></h4>
			</div>
			<?php endif; ?>
			...
			if ( ! empty( $product_cats ) && ! is_wp_error( $product_cats ) ) : ?>
				...
				<ul class="product-categories-list font-heading">
								...
								if ( $product_cats_children ) : ?>
									<ul class="product-child-categories pos-r">
										<?php foreach ( $product_cats_children as $product_cats_child ) :
											...
											?>
											<li class="product-category pos-r">
...
if ( class_exists( 'WooCommerce' ) ) {
	\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new SSD_Elementor_Product_Category_List_Widget() );
}