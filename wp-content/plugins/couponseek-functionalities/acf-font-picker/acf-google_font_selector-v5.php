...
class acf_field_google_font_selector extends acf_field {
	...
	function render_field( $field ) {
		...
		$current_font_family = ( empty( $field['value'] ) ) ? $field['default_font'] : $field['value']['family'];
		?>
	...
	function update_value( $value, $post_id, $field ) {
		$new_value = array();
		$new_value['family'] = $value;
		// if( empty( $_POST[$field['key'] . '_variants'] ) ) {
			$_POST[$field['key'] . '_variants'] = acfgfs_get_font_variant_array( $new_value['family'] );
		// }

		// if( empty( $_POST[$field['key'] . '_subsets'] ) ) {
			$_POST[$field['key'] . '_subsets'] = acfgfs_get_font_subset_array( $new_value['family'] );
		// }
